...
from fastapi import HTTPException, Form, Request, File, UploadFile, Cookie
...
def setup_admin_routes(app, database):
    ...
    async def admin_page(admin_session: str = Cookie(None)):
        ...
        <html>
        <head>
            <title>챌린지 관리자 시스템 (JSON 버전)</title>
            <meta charset="UTF-8">
    ...
    async def admin_login(password: str = Form(...)):
        if password == ADMIN_PASSWORD:
            ...
            response.set_cookie(key="admin_session", value=ADMIN_PASSWORD, max_age=86400)
            return response
        else:
    ...
    async def get_user_detail_page(nickname: str, admin_session: str = Cookie(None)):
        ...
        <html>
        <head>
            <title>{nickname} - 사용자 상세 정보 (JSON 버전)</title>
            <meta charset="UTF-8">
            <style>
        ...
        <body>
            <div class="container">
                <div class="header">
                    <h1>👤 {user_data['nickname']} 상세 정보 (JSON 버전)</h1>
                ...
                <div class="info-grid">
                    <div class="info-card">
                        ...
                        <div class="info-row">
                            ...
                            <span class="info-value">{user_data['created_at'][:10] if user_data['created_at'] != '알 수 없음' else '알 수 없음'}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">마지막 활동</span>
                            <span class="info-value">{user_data['last_activity'][:10] if user_data['last_activity'] != '없음' else '없음'}</span>
                        </div>
                    </div>
                    ...
                    <div class="info-card">
                        ...
                        {''.join([f'<div class="info-row"><span class="info-label">질문 {qid.split("_")[1]}</span><span class="info-value">{resp}</span></div>' for qid, resp in user_data['survey_responses'].items()]) if user_data['survey_responses'] else '<div class="activity-item">아직 설문에 참여하지 않았습니다</div>'}
                    </div>
                    ...
                    <div class="info-card">
                        ...
                        <div class="info-row">
                            ...
                            <span class="info-value">{user_data['video_progress']:.1f}%</span>
                        </div>
                        <div class="progress-bar" style="height: 10px;">
                            <div class="progress-fill" style="width: {user_data['video_progress']:.1f}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="info-card">
                        ...
                        <div class="info-row">
                            ...
                            <span class="info-value">{len(user_data['payment_activities'])}/7 회</span>
                        </div>
                        {''.join([f'<div class="activity-item">{activity["timestamp"][:10]}: {activity["amount"]:,}원 입금 💳</div>' for activity in user_data['payment_activities']]) if user_data['payment_activities'] else '<div class="activity-item">아직 결제 활동이 없습니다</div>'}
                    </div>
                </div>
            </div>
        </body>
